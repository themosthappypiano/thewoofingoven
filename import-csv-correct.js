        ...
        try {
          ...
          if (!products[handle]) {
            products[handle] = {
              ...
              description: row['Body (HTML)']?.replace(/<[^>]*>/g, '')?.trim() || 'Product description',
              price: row['Variant Price'] || '0',
              category: row.Type?.toLowerCase()?.includes('cake') ? 'cake' : 'treat'
            };
          }
          ...
          if (row['Variant Price'] && row['Variant SKU']) {
            const variant = {
              ...
              shipping_required: row['Variant Requires Shipping']?.toLowerCase() === 'true'
            };
